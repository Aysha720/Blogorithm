...
const AddBlog = () => {
    ...
    const generateContent = async () => {
        if(!title) return toast.error('Plaese enter a title')
            try {
                setLoading(true);
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex-1 bg-blue-50/50 text-gray-600 h-full overflow-scroll'>
        <div className='bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow rounded'>
            ...
            <button disabled={isAdding} type="submit" className='mt-8 w-40 h-10 bg-teal-500 text-white rounded cursor-pointer text-sm'>
